...
function LiveEntry(entryObject) {
    ...
    this.flavorsObjArray = createFlavorsObjArray(entryObject.flavorParamsIds.split(','));
...
function createFlavorsObjArray(flavorsArray) {
    return _.map(flavorsArray, (f) => {
        //flavorUrl =
        return { name : f };
    });
}

function onUpdateFlavorInfo(flavor, data) {
    let flavorObj = _.find(this.flavorsObjArray, (f) => {
        return f.name === flavor;
    });
    flavorObj.bandwidth = data.bitrate_kbps;
    flavorObj.resolution = data.resolution;
}
...
function createFlavorDownloaders() {
    ...
    var promises = _.map(that.flavorsDownloaders, function(f) {
        ...
        f.on('updateFlavorInfo', onUpdateFlavorInfo.bind(that));
