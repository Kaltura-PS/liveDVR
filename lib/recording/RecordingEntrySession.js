...
class RecordingEntrySession {
    ...
    parseUpdateJsonResult(result){
        ...
        this.touchPlaylistJson()
    }

    touchPlaylistJson(){

        touch(this.recordingSessionPath, {force: true}, (err)=>{
            if (err){
                this.logger.error("Failed to touch playlist json: %s", ErrorUtils.error2string(err))
                return
            }
            this.logger.info("Successfully  touch playlist json:")
